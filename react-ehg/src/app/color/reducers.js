...
import { SET_USER,
         ...
         VALIDATE_COLOR_CREATE, VALIDATE_COLOR_UPDATE, 
 ...
 * Dialog
 */
function createDialog(state = {isVisible: false, name: {}, color: {}, order: {}}, action) {
  switch (action.type) {
    ...
    case VALIDATE_COLOR_CREATE:
      if (isLength(trim(action.color), {min: 7, max: 9})) {
        return Object.assign({}, state, {color:{value: trim(action.color), valid: true}})
      } else {
        return Object.assign({}, state, {color:{value: action.color, valid: false, error: strings.error_prompt_color}})
      }
...
function updateDialog(state = {isVisible: false, name: {}, color: {}, order: {}}, action) {
  switch (action.type) {
      ...
      if (action.open) {
        return Object.assign({}, state,
          { isVisible: true, 
            ...
            color: {value: action.selectedRow.color, valid: true},
  ...
  switch (action.type) {
    case OPEN_DELETE_DIALOG:
      if (action.open) {
        ...
        content = {__html: '确认删除颜色：<span style="background-color:#ff0000; color:#ffffff;">' + tmp + '</span>吗？删除后将无法恢复！'}
