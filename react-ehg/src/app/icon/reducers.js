...
import { SET_USER,
         ...
         VALIDATE_ICON_CREATE, VALIDATE_ICON_UPDATE, 
 ...
 * Dialog
 */
function createDialog(state = {isVisible: false, name: {}, icon: {}, order: {}}, action) {
  switch (action.type) {
...
function updateDialog(state = {isVisible: false, name: {}, icon: {}, order: {}}, action) {
  switch (action.type) {
      ...
      if (action.open) {
        return Object.assign({}, state,
          { isVisible: true, 
            ...
            icon: {value: action.selectedRow.icon, valid: true},
  ...
  switch (action.type) {
    case OPEN_DELETE_DIALOG:
      if (action.open) {
        ...
        content = {__html: '确认删除图标：<span style="background-color:#ff0000; color:#ffffff;">' + tmp + '</span>吗？删除后将无法恢复！'}
