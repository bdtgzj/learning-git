...
import { SET_USER,
         ...
         SET_REGION_ID_CREATE, SET_REGION_ID_UPDATE, SET_REGION_ID_READ,
         SELECT_ROW,
         SET_REGIONS
       } from './actions'
// validator
 ...
 * Dialog
 */
function createDialog(state = {isVisible: false, name: {}, icon: {}, color: {}, region: {}, order: {}}, action) {
  switch (action.type) {
...
function readDialog(state = {isVisible: false, name: {}, region: {}}, action) {
  switch (action.type) {
...
function updateDialog(state = {isVisible: false, name: {}, icon: {}, color: {}, region: {}, order: {}}, action) {
  switch (action.type) {
  ...
  switch (action.type) {
    case OPEN_DELETE_DIALOG:
      if (action.open) {
        ...
        content = {__html: '确认删除场景：<span style="background-color:#ff0000; color:#ffffff;">' + tmp + '</span>吗？删除后将无法恢复！'}
