...
var Admin = Promise.promisifyAll(require('../proxy').Admin);
...
exports.signin = function(req, res, next) {
  new JSONAPIDeserializer().deserialize(req.body)
    .then(function(admin) {
      var name = validator.trim(admin.name);
      var password = validator.trim(admin.password);
      Admin.getAdminByNameEmailMPhonePass(name, hashcrypt.sha1(password), function(err, admins) {
        if (err) {
          return next(err);
        }
        var admin = admins.length > 0 ? admins[0] : null;
        res.json(AdminSerializer.serialize(admin));
